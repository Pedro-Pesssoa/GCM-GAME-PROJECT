...
from pergunta.models import Alternativa, Pergunta

# Dados das perguntas (copiadas do frontend)
perguntas_data = [
    {
        "id": 1,
...
def popular_perguntas():
    """Popula o banco de dados com as perguntas"""
    print("Iniciando população do banco de dados...")
    print(f"Total de perguntas a adicionar: {len(perguntas_data)}\n")

    # Limpar perguntas existentes (opcional)
    resposta = input("Deseja limpar as perguntas existentes? (s/n): ")
    if resposta.lower() == 's':
        Alternativa.objects.all().delete()
        Pergunta.objects.all().delete()
        print("✓ Perguntas existentes removidas\n")
    ...
    for data in perguntas_data:
        ...
        if created:
            perguntas_criadas += 1
            print(f"✓ Pergunta {data['id']}: {data['question'][:50]}...")
            ...
            print(f"- Pergunta {data['id']} já existe, pulando...")

    print(f"\n{'='*60}")
    print(f"✓ População concluída!")
    print(f"  - Perguntas criadas: {perguntas_criadas}")
    print(f"  - Alternativas criadas: {alternativas_criadas}")
    print(f"  - Total no banco: {Pergunta.objects.count()} perguntas")
    print(f"{'='*60}\n")
